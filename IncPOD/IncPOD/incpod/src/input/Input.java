...
public class Input {
	...
	public Input(RelationalInput relationalInput,RelationalInput incInput, int rowLimit,int incrowLimit) {
		...
		while (lineCount<rowLimit+incrowLimit) {
			...
			if(flag) {if(incInput.hasNext()) line = incInput.next();else System.out.println("incdata error");}
			else
				{if(relationalInput.hasNext()) line = relationalInput.next();else System.out.println("data error");}
